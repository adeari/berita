	...
	<table class="table table-striped jambo_table bulk_action" v-bind:class="blur">
	  ...
	  <tbody>
	    <tr v-show="beritas.length > 0" v-for="berita in beritas">
	      ...
	      <td><a href="{{ URL::to('admin-beritadetail-') }}@{{ berita.id }}-admin-berita?pagego=@{{ pagego }}&filter=@{{ filter }}&tampilkan=@{{ tampilkan }}&pencarian=@{{ pencarian }}" style="text-decoration:underline;">@{{ berita.judul }}</a></td>
